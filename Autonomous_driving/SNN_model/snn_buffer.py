...
class RecurrentRolloutBuffer(RolloutBuffer):
    ...
    def reset(self):
        super().reset()
        self.spike_states_pi = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.mem_states_pi = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.thresh_states_pi = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.step_num_pi = np.zeros(self.hidden_state_shape, dtype=np.float32)[...,0, np.newaxis]
        self.spike_states_vf = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.mem_states_vf = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.thresh_states_vf = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.step_num_vf = np.zeros(self.hidden_state_shape, dtype=np.float32)[...,0, np.newaxis]

    def add(self, *args, lstm_states: RNNStates, **kwargs) -> None:
        """
        SNN states
        """
        self.spike_states_pi[self.pos] = np.array(lstm_states.pi[0].cpu().numpy())
        self.mem_states_pi[self.pos] = np.array(lstm_states.pi[1].cpu().numpy())
        self.thresh_states_pi[self.pos] = np.array(lstm_states.pi[2].cpu().numpy())
        self.step_num_pi[self.pos] = np.array(lstm_states.pi[3].cpu().numpy())
        self.spike_states_vf[self.pos] = np.array(lstm_states.vf[0].cpu().numpy())
        self.mem_states_vf[self.pos] = np.array(lstm_states.vf[1].cpu().numpy())
        self.thresh_states_vf[self.pos] = np.array(lstm_states.vf[2].cpu().numpy())
        self.step_num_vf[self.pos] = np.array(lstm_states.vf[3].cpu().numpy())
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RecurrentRolloutBufferSamples, None, None]:
        ...
        if not self.generator_ready:
            ...
            for tensor in ["spike_states_pi", "mem_states_pi", "thresh_states_pi", "step_num_pi", "spike_states_vf", "mem_states_vf", "thresh_states_vf", "step_num_vf"]:
    ...
    def _get_samples(
        ...
        lstm_states_pi = (
            ...
            self.spike_states_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            self.mem_states_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            self.thresh_states_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            self.step_num_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        )
        lstm_states_vf = (
            # (n_envs * n_steps, n_layers, dim) -> (n_layers, n_seq, dim)
            self.spike_states_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            self.mem_states_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            self.thresh_states_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            self.step_num_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        )
        lstm_states_pi = (self.to_torch(lstm_states_pi[0]).contiguous(), self.to_torch(lstm_states_pi[1]).contiguous(), self.to_torch(lstm_states_pi[2]).contiguous(), self.to_torch(lstm_states_pi[3]).contiguous())
        lstm_states_vf = (self.to_torch(lstm_states_vf[0]).contiguous(), self.to_torch(lstm_states_vf[1]).contiguous(), self.to_torch(lstm_states_vf[2]).contiguous(), self.to_torch(lstm_states_vf[3]).contiguous())
