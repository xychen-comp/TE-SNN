...
if __name__ == "__main__":
    ...
    snn_ckp_dir = os.path.join(home_dir, 'exp/Duration/checkpoint/')
    snn_rec_dir = os.path.join(home_dir, 'exp/Duration/record/')
    ...
    def train(epoch):
        ...
        for i in range(0, X_train.size(0), batch_size):
            ...
            optimizer.zero_grad()
            output = net(x,'duration')
            MAE = (torch.round(output) - y).abs().mean()
